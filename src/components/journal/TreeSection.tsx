...
const GoodNote = ({ text, createdAt }: { text: string; createdAt: string }) => (
  <motion.div
    className="p-2 rounded-md bg-green-100/60 border border-green-400/50 text-sm"
...
const NotesGraph = ({ notes }: { notes: Note[] }) => {
  ...
  if (goodCount === 0 && badCount === 0) {
    return (
      <div className="text-center text-muted-foreground py-10 h-[200px] flex items-center justify-center">
        No notes yet to display graph.
      </div>
    );
  }
  return (
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
            ...
            notes.map((n) =>
              n.type === 'good' ? (
                <GoodNote
                  key={n.id}
                  text={n.text}
                  createdAt={n.createdAt?.toDate ? format(n.createdAt.toDate(), 'P p') : ''}
                />
              ) : (
                <BurningNote
                  key={n.id}
        ...
        <DialogFooter>
          <Button onClick={downloadPDF} disabled={notes.length === 0}>
              ...
              <Button
                onClick={async () => {
                  ...
                  try {
                    ...
                    setIsSavingBad(false);
                  }
                }}
                disabled={isSavingBad || !badNote}
              >
                {isSavingBad ? <Loader2 className="animate-spin" /> : 'Release'}
              </Button>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex space-x-2">
              ...
              <Button
                ...
                disabled={isSavingGood || !goodNote}
              >
